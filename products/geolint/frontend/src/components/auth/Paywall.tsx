...
export default function Paywall({ children }: Props) {
  ...
  const [storedEmail, setStoredEmail] = useState<string | null>(localStorage.getItem('geolint_email'))
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      if (res.data.url) {
        localStorage.setItem('geolint_email', email)
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50 p-6 dark:bg-gray-950">
      <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-8 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        ...
        <h2 className="mb-2 text-center text-xl font-bold">GeoLint is subscription-only</h2>
        <p className="mb-6 text-center text-sm text-gray-500 dark:text-gray-400">
          Start a $199/mo subscription to upload datasets, run validations, and view reports.
        </p>
        ...
        <div className="space-y-3">
          <input
            ...
            className="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-2 text-sm outline-none focus:border-accent dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100"
          />
          {storedEmail ? (
            <button
