...
app.get('/health', (c) => c.json({ status: 'healthy', service: 'eternalmap-worker' }))
...
crudRoutes('plots', 'plots', ['plot_id', 'section', 'status', 'occupant', 'deed'])
crudRoutes('deeds', 'deeds', ['deed_id', 'owner', 'plot', 'issued', 'status'])
crudRoutes('families', 'families', ['name', 'members', 'plots', 'portal'])

app.post('/api/v1/stripe/checkout', async (c) => {
  const stripe = new Stripe(c.env.STRIPE_SECRET_KEY, { apiVersion: '2024-04-10' as any })
  const body = await c.req.json()
  const session = await stripe.checkout.sessions.create({ mode: 'subscription', line_items: [{ price: c.env.STRIPE_PRICE_ID, quantity: 1 }], success_url: `${c.env.FRONTEND_URL}/?success=1`, cancel_url: `${c.env.FRONTEND_URL}/?canceled=1`, customer_email: body.email, metadata: { product: 'eternalmap' } })
...
app.post('/api/v1/stripe/webhook', async (c) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (email && customerId) await c.env.DB.prepare(`INSERT INTO subscriptions (id, customer_id, email, product, status, price_id, subscription_id) VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(randomUUID(), customerId, email, s.metadata?.product || 'eternalmap', 'active', s.line_items?.data[0]?.price?.id || c.env.STRIPE_PRICE_ID, s.subscription?.toString() || null).run()
  }
